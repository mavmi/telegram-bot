package mavmi.telegram_bot.rocket_bot.constants;

public class Phrases {
    public static final String GREETINGS_MSG = "Здравствуйте.";
    public static final String ENTER_LOGIN_DATA_MSG = "Введите данные в формате:\nлогин\nпароль";
    public static final String INVALID_DATA_FORMAT_MSG = "Невалидный формат данных";
    public static final String LOGIN_SUCCESS_MSG = "Авторизация прошла успешно";
    public static final String LOGOUT_SUCCESS_MSG = "Выход из учетки";
    public static final String LOGIN_FAIL_MSG = "Ошибка авторизации";
}
