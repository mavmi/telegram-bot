...
@Component
public class NoteGetEventGroupServiceModule implements ServiceModule<HbServiceResponse, HbServiceRequest> {

    private final CommonServiceModule commonServiceModule;
    private final HbServiceMessageToServiceMethodContainer container;

    public NoteGetEventGroupServiceModule(
    ...
    private HbServiceResponse onDefault(HbServiceRequest request) {
        ...
        String eventGroupName = request.getMessageJson().getTextMessage();
        if (eventGroupName.equals(commonServiceModule.getConstants().getButtons().getMenuBack())) {
        ...
        List<Event> events = commonServiceModule.getGoogleSheetsHandler().getAllEvents().getEventsByGroupName(eventGroupName);

        if (events == null) {
            MessageJson messageJson = MessageJson
                    .builder()
                    .textMessage(commonServiceModule.getConstants().getPhrases().getInvalidEventGroupName())
                    .build();
            ...
            dataCache.getMenuContainer().add(HbServiceMenu.NOTE_EVENT);
            dataCache.setHbSelectedEventGroup(eventGroupName);

            MessageJson messageJson = MessageJson
                    .builder()
                    .textMessage(commonServiceModule.getConstants().getPhrases().getAskForEventName())
                    .build();
            ReplyKeyboardJson replyKeyboardJson = ReplyKeyboardJson
                    .builder()
                    .keyboardButtons(commonServiceModule.prepareEventsKeyboardButtons(events))
                    .build();
            return HbServiceResponse
                    .builder()
                    .hbServiceTask(HB_SERVICE_TASK.SEND_KEYBOARD)
