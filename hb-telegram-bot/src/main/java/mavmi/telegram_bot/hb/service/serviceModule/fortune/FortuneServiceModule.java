...
@Component
public class FortuneServiceModule implements ServiceModule<HbServiceResponse, HbServiceRequest> {

    private final CommonServiceModule commonServiceModule;
    private final HbServiceMessageToServiceMethodContainer container;

    public FortuneServiceModule(
            CommonServiceModule commonServiceModule
    ) {
        this.commonServiceModule = commonServiceModule;
        this.container = new HbServiceMessageToServiceMethodContainer(
                Map.of(
                        commonServiceModule.getConstants().getRequests().getFortune(), this::onFortuneRequest
                )
        );
    }
    ...
    private HbServiceResponse onFortuneRequest(HbServiceRequest request) {
        commonServiceModule.getCacheComponent().getCacheBucket().getDataCache(HbDataCache.class).getMenuContainer().add(HbServiceMenu.FORTUNE_USERNAME);
